...
func main() {
	...
	httpdir := http.Dir(wd + "/web")
	_, err = os.Stat(string(httpdir))
	if err != nil {
		log.Printf("Cannot find serve directory %v", httpdir)
		return
	}
	web := http.FileServer(httpdir)
	log.Printf("Using '%v' for static files", httpdir)
